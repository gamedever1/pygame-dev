...
def loadTextures():
    ...
    for i in range(len(itemSprites1)): #для каждой нарезанной картинки-объекта, помещаем его в случайное место на карте
        gameObjectList.append(models.GameObject([randrange(0, xRes-itemSprites1[i].get_rect().w),randrange(0, yRes - itemSprites1[i].get_rect().h)], itemSprites1[i], items1_Names[i]))
    gameObjectList[len(gameObjectList)-1].followMouse = True
    ...
    for i in range(len(treeSprites)): #деревья также разбрасываем в случайные места на карте
        gameObjectList.append(models.GameObject([randrange(0, xRes-treeSprites[i].get_rect().w),randrange(0, yRes - treeSprites[i].get_rect().h)], treeSprites[i], 'Дерево'))   
